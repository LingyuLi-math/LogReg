...
Data1 = read.table("Data\\GSE73685_scale_blood_zf17_zhafter.txt", header = T, check.names = FALSE)
dim(Data1)    # 20910    17

gene = read.csv("Data37\\result\\gene_overlap_scale20.csv", header=TRUE, sep = ',') [,-1]
gene <- as.matrix(gene)
...
# write.table(genedata2,"Data\\GSE73685_17.txt",quote=F,sep="\t")  # 2020.6.29


##  extract data from origina discovery dataset
data1 = read.table("Data\\GSE59491_scale_DE.txt", header = T, check.names = FALSE)
dim(data1)    # 360 326
...
genedata2 <- rbind(data1[1,],genedata1)
rownames(genedata2) <- c('Lable', rownames(genedata1))
# write.table(genedata2,"Data\\GSE59491_17.txt",quote=F,sep="\t") 